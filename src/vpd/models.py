...
class AttentionControl(abc.ABC):
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str):
        if self.cur_att_layer >= self.num_uncond_att_layers:
            # only save text condition's attention maps context = [uncond, text]
            attn = self.forward(attn, is_cross, place_in_unet)
...
class AttentionStore(AttentionControl):
    ...
    def get_average_attention_across_time(self):
...
def register_BasicTransformerBlock(model):
    
    def transformer_block_forward(self):
        ...
        def forward(x, context=None):
            return checkpoint(_forward, (x, ), self.parameters(), self.checkpoint)
                                
        return forward
    
    def register_submodule(net_):
        if net_.__class__.__name__ == 'BasicTransformerBlock':
            del net_.attn2
            del net_.norm2
            net_.forward = transformer_block_forward(net_)
        elif hasattr(net_, 'children'):
            for net__ in net_.children():
                register_submodule(net__)
    
    for net in model.diffusion_model.named_children():
        if "input_blocks" in net[0] or "output_blocks" in net[0] or "middle_block" in net[0]:
            register_submodule(net[1])
...
class UNetWrapper(nn.Module):
    def __init__(self, sd_model, use_attn=True, store_att=False, base_size=512, max_attn_size=None, attn_selector='up_cross+down_cross') -> None:
        super().__init__()
        self.unet = sd_model.model
        ...
        register_BasicTransformerBlock(self.unet)
        if self.use_attn or self.store_att:
    ...
    def forward(self, *args, **kwargs):
        ...
        # extract text cond features only
        if self.use_attn:
            #avg_attn = self.attention_store.get_average_attention_across_time()
            avg_attn = self.attention_store.get_average_attention()
            attn16, attn32, attn64 = self.process_attn(avg_attn)
            # out_list[x] has text_cond
